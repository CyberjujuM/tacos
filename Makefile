...
LDLIBS=-lc -lutils -ldrivers -z nodefaultlib -lsystem -lstl
LDFLAGS=-Llib/
SUBDIRS = kernel libc utils drivers system libs/stl applications
...
kernel.bin: force_look
	...
	$(LD) -T linker.ld -o kernel.bin kernel/*.o kernel/drivers/*.o libs/stl/*.o -melf_i386 $(LDFLAGS) $(LDLIBS)
...
runqemunet: core.img
	qemu -fda core.img -vga std -soundhw pcspk -net nic,model=rtl8139,macaddr=AC:DC:DE:AD:BE:EF -net tap,ifname=tap0,script=no -net dump,file=eth.log -m 20 

runqemu: core.img
	qemu -fda core.img -vga std -soundhw pcspk -net nic,model=rtl8139,macaddr=AC:DC:DE:AD:BE:EF -net dump,file=eth.log -parallel none -m 20 
runqemugdb: core.img
	qemu -fda core.img -vga std -soundhw pcspk -net nic,model=rtl8139,macaddr=AC:DC:DE:AD:BE:EF -net dump,file=eth.log -parallel none -m 20 -s -S
	
runqemucurses: core.img
	qemu -fda core.img -vga std -curses -soundhw pcspk -net nic,model=rtl8139,macaddr=AC:DC:DE:AD:BE:EF -net dump,file=eth.log -m 20 
